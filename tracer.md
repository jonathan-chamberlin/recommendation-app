...
3. **Model**: Popularity baseline — finds the book with the highest average rating from the dataset and returns it.
4. **Backend (API)**: Sends the result back to the frontend.
5. **Frontend**: Displays the book's name, author, and average rating.

## Open Questions
- When does data loading happen? (On startup vs. per request)
